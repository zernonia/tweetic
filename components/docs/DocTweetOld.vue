...
const tweetOldRef = ref()
const highlightResponse = computed(() =>
  tweetOldRef.value?.data
    ? $hljs.highlight(JSON.stringify(tweetOldRef.value.data, null, "  "), { language: "json", ignoreIllegals: false })
        .value
    : ""
)
</script>
...
<template>
  <DocsBase method="GET" url="https://tweetic.io/api/tweets" description="Obtain static tweets (deprecated)">
    <template #config>
      ...
      <Toggle class="mt-2" name="show_original_link" v-model="params.show_original_link"> show_original_link </Toggle>
    ...
    <template #preview>
      <div class="flex justify-center">
        <TweetOld ref="tweetOldRef" v-bind="params"></TweetOld>
      </div>
